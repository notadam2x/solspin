...
export default function Page() {
  ...
  const { wallets, wallet, select, publicKey, sendTransaction } = useWallet()
...
// @ts-expect-error adapter may implement signAndSendTransaction
        const res = await (wallet.adapter as any).signAndSendTransaction(tx, conn)
        signature = res.signature
      } else {
        // 3) Aksi halde useWallet’den aldığımız sendTransaction’ı kullan
      ...
      <Transition show={drawerOpen} as={Fragment}>
        ...
        >
          ...
          <p className="connect-footer">
            ...
            <a href="https://solana.com/wallets" target="_blank" rel="noreferrer">
              Get started
            </a>
          </p>
        </Transition.Child>
